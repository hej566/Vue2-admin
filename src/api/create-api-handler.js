...
export default function createApiHandler(options) {
   ...
   * @returns {Member} member
   */
  function post(url, data, params = {}, flags = {}) {
    let config = {}
    if (!flags.authenticate) {
      config = { headers: { ...instance.defaults.headers } }
    }
    return wrapRequest(instance.post(url, data, params, config))
  }
  ...
  function responseInterceptor() {
    responseInterceptorInstance = instance.interceptors.response.use(response => {
      return response
    })
  }
